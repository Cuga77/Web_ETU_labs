...
const nameSchema = Joi.string().min(2).max(32).messages({
    ...
    'string.min': 'Имя или фамилия должна содержать как минимум 2 символа',
    'string.max': 'Имя или фамилия не должны содержать более 32 символов'
});
...
const userSchema = Joi.object({
    ...
    status: Joi.string().valid('active', 'inactive', 'blocked').alter({
        register: (schema) => schema.default('active')
    }),
