export class Database {
    ...
    async updateSet(query, updateSetData, multi = false) {
        try {
            return await this.collection.updateAsync(query, {$set: updateSetData}, {multi, returnUpdatedDocs: true});
        } catch (error) {
            throw new Error("Error updating document (query: " + query + "): " + error.message);
        }
    }

    async update(query, updateQuery, multi = false) {
        try {
            return await this.collection.updateAsync(query, updateQuery, {multi, returnUpdatedDocs: true});
        } catch (error) {
