import Joi from 'joi';
import {chats, posts, users} from '../models/collections.js';
import {
...
const getUserChats = async (req, res) => {
    try {
        const chatsList = await users.findById(req.params.id, {chats: 1});
        if (!chatsList)
            return res.status(404).json({error: "User not found"});

        let value = await chats.find({'_id': {$in: chatsList.chats}}, {messages: 0}, {updatedAt: -1});
        for (let item of value) {
            item.participants = await users.find({'_id': {$in: item.participants}}, {avatar: 1, firstName: 1, lastName: 1});
        }
        const totalCount = await chats.count({'_id': {$in: chatsList.chats}});
        return res.status(200).json({totalCount, value});
    } catch (error) {
        return res.status(500).json({error: error.message});
    }
...
const getUserPosts = async (req, res) => {
    try {
        const postsList = await users.findById(req.params.id, {posts: 1});
        if (!postsList)
            return res.status(404).json({error: "User not found"});

        let value = await posts.find({'_id': {$in: postsList.posts}}, {}, {updatedAt: -1});
        const totalCount = await posts.count({'_id': {$in: postsList.posts}});
        const authorIds = new Set(value.map(r => r.authorId));
        const authors = await users.find({_id: {$in: Array.from(authorIds)}}, {firstName: 1, lastName: 1, avatar: 1});

        return res.status(200).json({totalCount, posts: value, authors});
    } catch (error) {
        return res.status(500).json({error: error.message});
    }
}
...
export {registerUser, loginUser, getAllUsers, getUserChats, getUserPosts, getUserById, getUserFollowing, getUserFollowers, getUsersByQuery, updateUser, deleteUser};