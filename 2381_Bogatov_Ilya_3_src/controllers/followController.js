...
const followUser = async (req, res) => {
    ...
    let op = '$addToSet';
    if (followerUser.following.includes(value.followingId) && followingUser.followers.includes(value.id)) {
        op = '$pull';
    }

    // TODO: Транзакции, операция должна быть атомарной, но в NeDB такого функционала нет.
    const followerResult = await users.update({_id: followerUser._id}, {[op]: {following: followingUser._id}});
    const followingResult = await users.update({_id: followingUser._id}, {[op]: {followers: followerUser._id}});

    if (followerResult.numAffected === 0 || followingResult.numAffected === 0) {
        return res.status(500).json({error: "Error in updating query."});
    }

    const hiddenResult = publicUserSchema.validate(followingResult.affectedDocuments, {allowUnknown: true});
